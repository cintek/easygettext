...
describe('Javascript extractor object', () => {
  describe('Extraction of localized strings', () => {
    it('should extract strings localized using $gettext from the script', () => {
      ...
      const extractedStrings = extractStringsFromJavascript(
        filename,
        VUE_COMPONENT_EXPECTED_PROCESSED_SCRIPT_TAG,
      );
      ...
      expect(firstString.reference.line).to.equal(5);
      ...
      expect(secondString.reference.line).to.equal(8);
      ...
      expect(thirdString.reference.line).to.equal(11);
    });
  ...
  describe('Multiple parser support', () => {
    it('should extract strings using the parser acorn', () => {
      ...
      expect(extractedStrings.length).to.equal(3);
      expect(extractedStrings[0].msgid).to.equal('Hello there!');
    });
